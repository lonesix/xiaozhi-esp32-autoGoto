...
#define TRIGGER_THRESHOLD      7.0f            //deg
...
static bmi270_axis_t app_imu_read(void)
{
    ...
    // ESP_LOGI(TAG, "%5.2f, %5.2f, %5.2f", axis_offset.pitch, axis_offset.yaw, axis_offset.roll);
    return axis_offset;
}
...
//     }
// }
// static void imu_check_offset(bmi270_axis_t axis_offset)
// {
//     // ESP_LOGI(TAG, "pitch,yaw: %5.2f, %5.2f", axis_offset.pitch, axis_offset.yaw);
//     if ((axis_offset.pitch < -TRIGGER_THRESHOLD) && !gesup_sentalready) {
//         // uint8_t key = LV_KEY_UP;
...
#define SHAKE_TIME_WINDOW       1000     // 摇晃检测时间窗口（毫秒）
#define SHAKE_RESET_TIME        2000     // 摇晃重置时间（毫秒）

// 全局状态变量
static bool roll_right_sentalready = false;
static bool roll_left_sentalready = false;
static bool flip_forward_sentalready = false;
static bool flip_backward_sentalready = false;
static bool shake_sentalready = false;
static bool combo_action_sentalready = false;

static void imu_check_offset(bmi270_axis_t axis_offset)
{
    // ESP_LOGI(TAG, "pitch,yaw,roll: %5.2f, %5.2f, %5.2f", axis_offset.pitch, axis_offset.yaw, axis_offset.roll);
    ...
    // // 左右翻滚检测 (Roll)
    // if ((axis_offset.roll > ROLL_THRESHOLD) && !roll_right_sentalready) {
    //     ESP_LOGI(TAG, "roll right %5.2f", axis_offset.roll);
    //     // 处理向右翻滚事件
    //     roll_right_sentalready = true;
    // } else if ((axis_offset.roll < -ROLL_THRESHOLD) && !roll_left_sentalready) {
    //     ESP_LOGI(TAG, "roll left %5.2f", axis_offset.roll);
    //     // 处理向左翻滚事件
    //     roll_left_sentalready = true;
    // } else {
    //     if (fabs(axis_offset.roll) < 0.3 * ROLL_THRESHOLD) {
    //         roll_right_sentalready = false;
    //         roll_left_sentalready = false;
    //     }
    // }
    
    // // 快速翻滚检测（前后翻滚）
    // if ((axis_offset.pitch > FLIP_THRESHOLD) && !flip_forward_sentalready) {
    //     ESP_LOGI(TAG, "flip forward %5.2f", axis_offset.pitch);
    //     // 处理前翻滚事件
    //     flip_forward_sentalready = true;
    // } else if ((axis_offset.pitch < -FLIP_THRESHOLD) && !flip_backward_sentalready) {
    //     ESP_LOGI(TAG, "flip backward %5.2f", axis_offset.pitch);
    //     // 处理后翻滚事件
    //     flip_backward_sentalready = true;
    // } else {
    //     if (fabs(axis_offset.pitch) < 0.3 * FLIP_THRESHOLD) {
    //         flip_forward_sentalready = false;
    //         flip_backward_sentalready = false;
    //     }
    // }
    
    // 摇晃检测
    static unsigned int shake_count = 0;
    static unsigned int last_shake_time = 0;
    uint32_t current_time = esp_timer_get_time() / 1000; // 获取当前时间（毫秒）
    
    // 检测是否有显著的运动变化
    float total_motion = sqrt(axis_offset.pitch * axis_offset.pitch + 
                             axis_offset.yaw * axis_offset.yaw + 
                             axis_offset.roll * axis_offset.roll);
    
    if (total_motion > SHAKE_THRESHOLD) {
        if (current_time - last_shake_time < SHAKE_TIME_WINDOW) {
            shake_count++;
        } else {
            shake_count = 1; // 重新开始计数
        }
        last_shake_time = current_time;
        
        // 如果在时间窗口内检测到足够多的摇晃
        if (shake_count >= SHAKE_COUNT_THRESHOLD && !shake_sentalready) {
            ESP_LOGI(TAG, "shake detected! count: %d, motion: %5.2f", shake_count, total_motion);
            // 处理摇晃事件
            auto& app = Application::GetInstance();
            
            app.ToggleChatState();
            shake_sentalready = true;
            shake_count = 0; // 重置计数
        }
    }
    
    // 重置摇晃状态
    if (current_time - last_shake_time > SHAKE_RESET_TIME) {
    ...
    if (exceeded_count >= 2) {
        if (!combo_action_sentalready) {
            ...
            app.ToggleChatState();
            combo_action_sentalready = true;
        }
    } else {
        if ((fabs(axis_offset.roll) < 0.3 * ROLL_THRESHOLD) && (fabs(axis_offset.yaw) < 0.3 * TRIGGER_THRESHOLD)) {
