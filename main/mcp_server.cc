...
void McpServer::AddCommonTools() {
    ...
    //         });
    //     AddTool("self.camera.sleep",
    //         "改变摄像头休眠状态\n"
    //         "Args:\n"
    //         "  `state`: 1:sleep,0:no sleep\n"
    //         "Return:\n"
    //         "  关于操作是否成功的json.",
    //         PropertyList({
    //             Property("state", kPropertyTypeInteger)
    //         }),
    //         [camera](const PropertyList& properties) -> ReturnValue {
    //             auto state = properties["state"].value<int>();
    //             if (!camera->Setsleep(state)) {
    //                 return "{\"success\": false, \"message\": \"Failed to sleep cam\"}";
    //             }
